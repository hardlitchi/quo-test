...
class BookServiceTest {
    ...
    @Test
    @DisplayName("書籍更新時に存在しない著者を指定すると例外が発生する")
    fun throwsExceptionWhenUpdatingBookWithNonExistentAuthor() {
        // Given: 書籍を登録
        bookService.createBook(validBookDto)
    ...
    @Test
    @DisplayName("書籍更新時に新しい著者を追加できる")
    fun canUpdateBookWithNewAuthor() {
        // Given: 書籍を登録
        bookService.createBook(validBookDto)
        
        // 新しい著者を登録
        val newAuthorDto = CreateAuthorDto(
            name = "芥川龍之介",
            birthDate = LocalDate.of(1892, 3, 1)
        )
        ...
        val updateDto = UpdateBookDto(
            ...
            price = BigDecimal("600"),
            publicationStatus = PublicationStatus.PUBLISHED,
            authors = listOf("夏目漱石", "芥川龍之介") // 新しい著者を追加
        )
    ...
    @Test
    @DisplayName("書籍更新時に既存の著者を削除できる")
    fun canUpdateBookWithAuthorRemoval() {
        // Given: 複数著者の書籍を登録
        val author2Dto = CreateAuthorDto(
            name = "芥川龍之介",
            birthDate = LocalDate.of(1892, 3, 1)
        ...
        assertThat(authors).containsExactly("夏目漱石")
    }

    @Test
    @DisplayName("空白の著者名で登録しようとすると例外が発生する")
    fun throwsExceptionWhenCreatingBookWithBlankAuthorName() {
        // Given
        val invalidDto = validBookDto.copy(authors = listOf("夏目漱石", "  ")) // 空白の著者名
